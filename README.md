...
| OAuth dynamic client registration (RFC 7591) | `parseOAuthClientRegistrationRequest`, `formatOAuthClientRegistrationRequest`, `validateOAuthClientRegistrationRequest`, `parseOAuthClientRegistrationResponse`, `formatOAuthClientRegistrationResponse`, `parseOAuthClientRegistrationErrorResponse`, `mergeSoftwareStatementClientMetadata` |
| OAuth dynamic client registration management (RFC 7592) | `parseOAuthClientConfigurationResponse`, `formatOAuthClientConfigurationResponse`, `validateOAuthClientConfigurationResponse`, `parseOAuthClientConfigurationUpdateRequest`, `formatOAuthClientConfigurationUpdateRequest`, `validateOAuthClientConfigurationUpdateRequest` |
...
- Auth and transport security: RFC 7617, RFC 6750, RFC 7616, RFC 7636, RFC 7591, RFC 7592, RFC 8414, RFC 6797, RFC 8470, RFC 7838, RFC 9053, RFC 9421, RFC 9309, RFC 9116, RFC 7239.
...
- WebAuthn RP ID validation rejects IP literals by default; use `allowIpRpId: true` only for explicit compatibility scenarios (for example, local test harnesses).
- `formatWebauthnClientDataJson` enforces HTTPS-origin defaults; pass `{ requireHttpsOrigin: false }` only for explicit non-production compatibility behavior.
